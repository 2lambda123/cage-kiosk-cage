...
static bool view_at(struct cg_view *view, double lx, double ly,
 ...
 * both or nothing. */
static struct cg_view *desktop_view_at(struct cg_server *server, double lx,
...
static void press_cursor_button(struct cg_seat *seat,
                                struct wlr_input_device *device, uint32_t time,
                                uint32_t button, uint32_t state, double lx,
                                double ly) {
...
static void update_capabilities(struct cg_seat *seat) {
  uint32_t caps = 0;
  ...
  if ((caps & WL_SEAT_CAPABILITY_POINTER) == 0) {
    ...
    wlr_xcursor_manager_set_cursor_image(seat->xcursor_manager, DEFAULT_XCURSOR,
                                         seat->cursor);
  }
...
static void handle_new_touch(struct cg_seat *seat,
...
static void handle_new_pointer(struct cg_seat *seat,
                               struct wlr_input_device *device) {
...
static bool handle_keybinding(struct cg_server *server, xkb_keysym_t sym) {
  switch (sym) {
#ifdef DEBUG
  case XKB_KEY_Escape:
...
static void handle_keyboard_key(struct wl_listener *listener, void *data) {
  ...
  if (!handled) {
    ...
    wlr_seat_keyboard_notify_key(seat->seat, event->time_msec, event->keycode,
                                 event->state);
  }
...
static void handle_new_keyboard(struct cg_seat *seat,
                                struct wlr_input_device *device) {
...
static void handle_touch_down(struct wl_listener *listener, void *data) {
  ...
  if (serial && wlr_seat_touch_num_points(seat->seat) == 1) {
    ...
    seat->touch_lx = lx;
    seat->touch_ly = ly;
    press_cursor_button(seat, event->device, event->time_msec, BTN_LEFT,
                        WLR_BUTTON_PRESSED, lx, ly);
  }
...
static void handle_touch_up(struct wl_listener *listener, void *data) {
  ...
  if (wlr_seat_touch_num_points(seat->seat) == 1) {
    press_cursor_button(seat, event->device, event->time_msec, BTN_LEFT,
                        WLR_BUTTON_RELEASED, seat->touch_lx, seat->touch_ly);
  }
...
static void handle_touch_motion(struct wl_listener *listener, void *data) {
  ...
  if (view) {
    wlr_seat_touch_point_focus(seat->seat, surface, event->time_msec,
                               event->touch_id, sx, sy);
    wlr_seat_touch_notify_motion(seat->seat, event->time_msec, event->touch_id,
                                 sx, sy);
  } else {
...
static void handle_cursor_axis(struct wl_listener *listener, void *data) {
  ...
  wlr_seat_pointer_notify_axis(seat->seat, event->time_msec, event->orientation,
...
static void handle_cursor_button(struct wl_listener *listener, void *data) {
  ...
  wlr_seat_pointer_notify_button(seat->seat, event->time_msec, event->button,
                                 event->state);
  press_cursor_button(seat, event->device, event->time_msec, event->button,
                      event->state, seat->cursor->x, seat->cursor->y);
  wlr_idle_notify_activity(seat->server->idle, seat->seat);
}

static void process_cursor_motion(struct cg_seat *seat, uint32_t time) {
  double sx, sy;
...
static void drag_icon_damage(struct cg_drag_icon *drag_icon) {
  struct cg_output *output;
